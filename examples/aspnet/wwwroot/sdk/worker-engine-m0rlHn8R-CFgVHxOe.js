import { h as _, x as b } from "./browser-BKLM0ThC-IxjwuOIz-C3aNmir5.js";
import { p as P, f, I as g, W as D } from "./index-Cz4mhCid.js";
const a = "RemoteExecutor", i = "Worker", F = class c {
  constructor(n, e) {
    this.worker = n, this.pendingRequests = /* @__PURE__ */ new Map(), this.requestCounter = 0, this.handleMessage = (t) => {
      const r = t.data;
      if (r.type === "ready") return this.logger.debug(a, i, "Worker is ready"), void this.readyTask.resolve(!0);
      const o = this.pendingRequests.get(r.id);
      if (o) switch (r.type) {
        case "result":
          this.logger.debug(a, i, `Received result for ${r.id}`), o.resolve(r.data), this.pendingRequests.delete(r.id);
          break;
        case "error":
          this.logger.debug(a, i, `Received error for ${r.id}:`, r.error), r.error ? o.fail(r.error) : o.reject({ code: P.Unknown, message: "Unknown error" }), this.pendingRequests.delete(r.id);
          break;
        case "progress":
          this.logger.debug(a, i, `Received progress for ${r.id}`), o.progress(r.progress);
      }
      else this.logger.warn(a, i, `Received response for unknown request: ${r.id}`);
    }, this.logger = e.logger ?? new f(), this.worker.addEventListener("message", this.handleMessage), this.readyTask = new g(), this.pendingRequests.set(c.READY_TASK_ID, this.readyTask), this.worker.postMessage({ id: c.READY_TASK_ID, type: "wasmInit", wasmUrl: e.wasmUrl, logger: e.logger ? D(e.logger) : void 0, fontFallback: e.fontFallback }), this.logger.debug(a, i, "RemoteExecutor created");
  }
  generateId() {
    return `req-${Date.now()}-${this.requestCounter++}`;
  }
  send(n, e) {
    const t = this.generateId(), r = new g(), o = { id: t, type: "execute", method: n, args: e };
    return this.readyTask.wait(() => {
      this.pendingRequests.set(t, r), this.logger.debug(a, i, `Sending ${n} request:`, t), this.worker.postMessage(o);
    }, (l) => {
      this.logger.error(a, i, `Worker init failed, rejecting ${n}:`, l), r.reject({ code: P.Initialization, message: "Worker initialization failed" });
    }), r;
  }
  destroy() {
    this.worker.removeEventListener("message", this.handleMessage), this.pendingRequests.forEach((n, e) => {
      e !== c.READY_TASK_ID && (n.abort("Worker destroyed"), this.logger.debug(a, i, `Aborted pending request: ${e}`));
    }), this.pendingRequests.clear(), this.worker.terminate(), this.logger.debug(a, i, "RemoteExecutor destroyed");
  }
  openDocumentBuffer(n, e) {
    return this.send("openDocumentBuffer", [n, e]);
  }
  getMetadata(n) {
    return this.send("getMetadata", [n]);
  }
  setMetadata(n, e) {
    return this.send("setMetadata", [n, e]);
  }
  getDocPermissions(n) {
    return this.send("getDocPermissions", [n]);
  }
  getDocUserPermissions(n) {
    return this.send("getDocUserPermissions", [n]);
  }
  getSignatures(n) {
    return this.send("getSignatures", [n]);
  }
  getBookmarks(n) {
    return this.send("getBookmarks", [n]);
  }
  setBookmarks(n, e) {
    return this.send("setBookmarks", [n, e]);
  }
  deleteBookmarks(n) {
    return this.send("deleteBookmarks", [n]);
  }
  renderPageRaw(n, e, t) {
    return this.send("renderPageRaw", [n, e, t]);
  }
  renderPageRect(n, e, t, r) {
    return this.send("renderPageRect", [n, e, t, r]);
  }
  renderThumbnailRaw(n, e, t) {
    return this.send("renderThumbnailRaw", [n, e, t]);
  }
  renderPageAnnotationRaw(n, e, t, r) {
    return this.send("renderPageAnnotationRaw", [n, e, t, r]);
  }
  renderPageAnnotationsRaw(n, e, t) {
    return this.send("renderPageAnnotationsRaw", [n, e, t]);
  }
  getPageAnnotationsRaw(n, e) {
    return this.send("getPageAnnotationsRaw", [n, e]);
  }
  getPageAnnotations(n, e) {
    return this.send("getPageAnnotations", [n, e]);
  }
  createPageAnnotation(n, e, t, r) {
    return this.send("createPageAnnotation", [n, e, t, r]);
  }
  updatePageAnnotation(n, e, t, r) {
    return this.send("updatePageAnnotation", [n, e, t, r]);
  }
  removePageAnnotation(n, e, t) {
    return this.send("removePageAnnotation", [n, e, t]);
  }
  getPageTextRects(n, e) {
    return this.send("getPageTextRects", [n, e]);
  }
  searchInPage(n, e, t, r) {
    return this.send("searchInPage", [n, e, t, r]);
  }
  getAnnotationsBatch(n, e) {
    return this.send("getAnnotationsBatch", [n, e]);
  }
  searchBatch(n, e, t, r) {
    return this.send("searchBatch", [n, e, t, r]);
  }
  getAttachments(n) {
    return this.send("getAttachments", [n]);
  }
  addAttachment(n, e) {
    return this.send("addAttachment", [n, e]);
  }
  removeAttachment(n, e) {
    return this.send("removeAttachment", [n, e]);
  }
  readAttachmentContent(n, e) {
    return this.send("readAttachmentContent", [n, e]);
  }
  getDocumentJavaScriptActions(n) {
    return this.send("getDocumentJavaScriptActions", [n]);
  }
  getPageAnnoWidgets(n, e) {
    return this.send("getPageAnnoWidgets", [n, e]);
  }
  getPageWidgetJavaScriptActions(n, e) {
    return this.send("getPageWidgetJavaScriptActions", [n, e]);
  }
  setFormFieldValue(n, e, t, r) {
    return this.send("setFormFieldValue", [n, e, t, r]);
  }
  setFormFieldState(n, e, t, r) {
    return this.send("setFormFieldState", [n, e, t, r]);
  }
  renameWidgetField(n, e, t, r) {
    return this.send("renameWidgetField", [n, e, t, r]);
  }
  shareWidgetField(n, e, t, r, o) {
    return this.send("shareWidgetField", [n, e, t, r, o]);
  }
  regenerateWidgetAppearances(n, e, t) {
    return this.send("regenerateWidgetAppearances", [n, e, t]);
  }
  flattenPage(n, e, t) {
    return this.send("flattenPage", [n, e, t]);
  }
  extractPages(n, e) {
    return this.send("extractPages", [n, e]);
  }
  createDocument(n) {
    return this.send("createDocument", [n]);
  }
  importPages(n, e, t, r) {
    return this.send("importPages", [n, e, t, r]);
  }
  deletePage(n, e) {
    return this.send("deletePage", [n, e]);
  }
  extractText(n, e) {
    return this.send("extractText", [n, e]);
  }
  redactTextInRects(n, e, t, r) {
    return this.send("redactTextInRects", [n, e, t, r]);
  }
  applyRedaction(n, e, t) {
    return this.send("applyRedaction", [n, e, t]);
  }
  applyAllRedactions(n, e) {
    return this.send("applyAllRedactions", [n, e]);
  }
  flattenAnnotation(n, e, t) {
    return this.send("flattenAnnotation", [n, e, t]);
  }
  exportAnnotationAppearanceAsPdf(n, e, t) {
    return this.send("exportAnnotationAppearanceAsPdf", [n, e, t]);
  }
  exportAnnotationsAppearanceAsPdf(n, e, t) {
    return this.send("exportAnnotationsAppearanceAsPdf", [n, e, t]);
  }
  getTextSlices(n, e) {
    return this.send("getTextSlices", [n, e]);
  }
  getPageGlyphs(n, e) {
    return this.send("getPageGlyphs", [n, e]);
  }
  getPageGeometry(n, e) {
    return this.send("getPageGeometry", [n, e]);
  }
  getPageTextRuns(n, e) {
    return this.send("getPageTextRuns", [n, e]);
  }
  merge(n) {
    return this.send("merge", [n]);
  }
  mergePages(n) {
    return this.send("mergePages", [n]);
  }
  preparePrintDocument(n, e) {
    return this.send("preparePrintDocument", [n, e]);
  }
  saveAsCopy(n) {
    return this.send("saveAsCopy", [n]);
  }
  closeDocument(n) {
    return this.send("closeDocument", [n]);
  }
  closeAllDocuments() {
    return this.send("closeAllDocuments", []);
  }
  setDocumentEncryption(n, e, t, r) {
    return this.send("setDocumentEncryption", [n, e, t, r]);
  }
  removeEncryption(n) {
    return this.send("removeEncryption", [n]);
  }
  unlockOwnerPermissions(n, e) {
    return this.send("unlockOwnerPermissions", [n, e]);
  }
  isEncrypted(n) {
    return this.send("isEncrypted", [n]);
  }
  isOwnerUnlocked(n) {
    return this.send("isOwnerUnlocked", [n]);
  }
};
F.READY_TASK_ID = "0";
let E = F;
const s = "ImageEncoderPool", d = "Encoder";
class A {
  constructor(n = 2, e, t) {
    this.poolSize = n, this.workerUrl = e, this.workers = [], this.pendingTasks = /* @__PURE__ */ new Map(), this.nextWorkerId = 0, this.requestCounter = 0, this.logger = t ?? new f(), this.initialize();
  }
  initialize() {
    this.logger.debug(s, d, `Creating worker pool with ${this.poolSize} workers`);
    for (let n = 0; n < this.poolSize; n++) try {
      const e = new Worker(this.workerUrl, { type: "module" });
      e.onmessage = this.handleWorkerMessage.bind(this), e.onerror = this.handleWorkerError.bind(this), this.workers.push(e), this.logger.debug(s, d, `Worker ${n} created successfully`);
    } catch (e) {
      this.logger.error(s, d, `Failed to create worker ${n}:`, e);
    }
  }
  handleWorkerMessage(n) {
    const e = n.data, t = this.pendingTasks.get(e.id);
    if (t) if (this.pendingTasks.delete(e.id), e.type === "result") t.resolve(e.data);
    else {
      const r = e.data;
      t.reject(new Error(r.message));
    }
    else this.logger.warn(s, d, `Received response for unknown task: ${e.id}`);
  }
  handleWorkerError(n) {
    this.logger.error(s, d, "Worker error:", n.message);
  }
  getNextWorker() {
    if (this.workers.length === 0) return null;
    const n = this.workers[this.nextWorkerId];
    return this.nextWorkerId = (this.nextWorkerId + 1) % this.workers.length, n;
  }
  encode(n, e = "image/png", t) {
    return new Promise((r, o) => {
      const l = this.getNextWorker();
      if (!l) return void o(new Error("No workers available in the pool"));
      const u = `encode-${Date.now()}-${this.requestCounter++}`;
      this.pendingTasks.set(u, { resolve: r, reject: o });
      const p = { id: u, type: "encode", data: { imageData: { data: n.data, width: n.width, height: n.height }, imageType: e, quality: t } };
      this.logger.debug(s, d, `Sending encoding request ${u} (${n.width}x${n.height})`), l.postMessage(p, [n.data.buffer]);
    });
  }
  destroy() {
    this.logger.debug(s, d, "Destroying worker pool"), this.pendingTasks.forEach((n, e) => {
      n.reject(new Error("Worker pool destroyed")), this.logger.debug(s, d, `Rejected pending task: ${e}`);
    }), this.pendingTasks.clear(), this.workers.forEach((n, e) => {
      n.terminate(), this.logger.debug(s, d, `Worker ${e} terminated`);
    }), this.workers = [];
  }
  get activeWorkers() {
...
function S(m, n) {
  const e = n instanceof Object && "debug" in n ? { logger: n } : n || {}, { logger: t, encoderPoolSize: r, fontFallback: o } = e, l = new Worker(URL.createObjectURL(new Blob([`var Rotation = /* @__PURE__ */ ((Rotation2) => {
...
`], { type: "application/javascript" })), h = new A(r ?? 2, p, t);
  return new _(u, { imageConverter: b(h), logger: t });
}
export {
  S as createPdfiumEngine
};
